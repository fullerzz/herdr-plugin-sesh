...
type ConfigSessions struct{ Config config.Config }
...
func (s ConfigSessions) List(context.Context) (model.Sessions, error) {
	...
	for _, c := range s.Config.SessionConfigs {
		sess := model.Session{Source: "config", Name: c.Name, Path: c.Path, StartupCommand: c.StartupCommand, PreviewCommand: c.PreviewCommand, DisableStartupCommand: c.DisableStartCommand, WindowNames: c.Windows}
